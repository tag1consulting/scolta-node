...
### Changed
- **Scolta ships with no AI provider selected, and `AutoProvisioner.ensureAiAvailable()` no longer mints a trial (`src/config.ts`, `src/health.ts`, `src/ai/client.ts`, `src/ai/service.ts`, `src/ai/amazee-service.ts`, `src/ai/amazee/auto-provisioner.ts`).** Two invariants, previously true in the PHP core and only half-true here. **No default provider:** `ScoltaConfig.ai_provider` defaulted to `"anthropic"`, `HealthChecker` coalesced an empty value back to `"anthropic"`, and `AiClient` defaulted its `provider` key the same way — so an install nobody had configured reported itself as an Anthropic install, and a key set in the environment before anyone chose a provider looked like a working one. The default is now `""`, nothing coalesces it, and `""` means AI is off: `AiServiceAdapter.getClient()` refuses to build a client and throws the `ApiKeyMissingError` the endpoint handlers already degrade on (unexpanded query, no summary) rather than picking a vendor on the site's behalf; `AiClient` rejects an absent provider outright; health reports `aiProvider: ""` and a new `aiProviderSelected: false`, and `aiUsable` is false whatever else is present. Going-forward only — a provider already persisted by a site is read as-is and never rewritten. **No auto-enable:** `ensureAiAvailable()` still constructed an `AmazeeTrialProvisioner` and minted a free trial whenever it found an empty credential store, and its own docstring declared the wrong contract outright ("the gate is the absence of an explicit key / stored credentials — NOT the provider name"), so any request or startup path that called it enrolled a site that had never opted in. That branch and that docstring are gone. The method now matches the PHP core exactly: it self-heals credentials that are already stored but whose model names were never resolved, and does nothing else.
- **`ai_provider = "amazee"` is the opt-in that permits first-use demo provisioning (`src/ai/amazee-service.ts`).** These frameworks have no admin UI, so a developer writing that value in configuration is the manual opt-in — the same act as clicking "Try the demo" in a CMS admin — and it is now the gate. `AmazeeAiService.buildClient()` establishes a demo connection with an explicit `AmazeeTrialProvisioner.provision()` call only when the provider is `amazee` and the credential store is empty; with the provider unset or set to anything else it makes no outbound Amazee call at all, and an explicit `ai_api_key` still wins and suppresses Amazee entirely. Provisioning is idempotent — the store being empty is the only trigger, so later requests reuse the stored credentials. Covered by `tests/ai/manual-provider-and-opt-in.test.ts`, whose Amazee fetches fail the test if they are called.
- **Which action established an Amazee.ai connection is recorded when it happens (`src/ai/amazee/connection-source.ts`, `storage.ts`, `trial-provisioner.ts`, `account-upgrader.ts`).** Nothing recorded whether a stored token came from the demo or from an operator's own account — both write the same three fields through `ConfigStorage.store()` — so any surface naming one was guessing. `AmazeeConnectionSource` (`demo` / `account`) is now written by the class that establishes the connection, through optional `storeConnectionSource` / `loadConnectionSource` members on `ConfigStorage`. Optional so every existing implementation keeps working untouched and simply reports no origin, which is the honest answer for credentials connected before this release. `FilesystemConfigStorage` and `MemoryConfigStorage` implement both, and drop the record in `clear()`.

### Removed
- **`AutoProvisioner.ensureAiAvailable()` no longer provisions anything, and its return value is always `false`.** It previously returned `true` when it had minted a fresh trial. There is no longer a success to report. The signature is unchanged, so callers keep compiling; a caller that branched on `true` should move that branch to its own explicit `provision()` call.

### Fixed
- **Prompt templates re-synced from scolta-core: the `expand_query` decomposition preference and rule 13 (`src/ai/prompts.generated.ts`).** Rule 13 is now `CATEGORY → INSTANCES` rather than `CATEGORY → MEMBERS`, and it no longer presupposes a closed set. It previously licensed decomposition only when the model could "name the members confidently", so open-ended groupings — regulations, platforms, regional cuisines — fell back to restating the category, and only closed canonical sets decomposed at all. It now states that a grouping needs no complete or canonical membership and that the several most prominent instances are a decomposition, and it forbids substituting a narrower grouping for the one asked about ("citrus fruits" → "tart fruits"), which is only the same query restated. The two category→sub-category examples that taught that exact shape ("European cars" → "German cars", "Southeast Asian food" → "Thai") are gone. The task definition itself now names both kinds of expansion — an alternate PHRASING and a DECOMPOSITION — and says decomposition is the stronger one wherever it is available; the definition is what governs here, since edits confined to the rules moved nothing on their own. That preference defers explicitly to rule 15, without which the pressure to name instances overrode the unrecognized-entity guard and invented an identity for a mission it did not recognize. Rule 14 gains the same open-ended allowance, and the Examples block gains three worked decompositions ("citrus fruits", "camping trip essentials", "onboarding a new hire"). Byte-identity with scolta-core is enforced by `tests/ai/prompt-identity.test.ts`.
